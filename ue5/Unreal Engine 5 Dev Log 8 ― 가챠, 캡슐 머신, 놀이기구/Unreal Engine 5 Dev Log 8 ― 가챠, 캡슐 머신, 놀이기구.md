...
# 작업 순서
1. 코인 스폰/줍기 + WBP 카운트
2. Insert  존 +  insertedCount 증가 + Pull 활성화
3. Pull 누르면 "입력 잠금 + 3초 후 캡슐 스폰" 같은 더미로 먼저 완성
4. 그 다음에 Level Sequence로 연출 갈아끼우기
5. 캡슐 열기 + 결과 위젯
---
# 배운점
...
#  오늘 한 일
1. 시네마틱 영상 작업 시작. 
	1. Timeline을 이용해서 Cinematic이 시작되면 machineobject가 rotate하게 함
2. 

---
# 개선할 부분
1. 코인을 하나만 넣었다가 나오고 다시 들어가면 타이머가 꼬임
    BeginOverlap 초입에 Timer Clear 함
2. 접근할 수 없는 위치에 생성되는 Coin
